...
class CoverLetterCreateRequest(BaseModel):
    ...
    job_id: Optional[UUID] = None
    title: str = Field(..., min_length=1, max_length=255)
    tone: str = Field(default="formal", regex="^(formal|casual|enthusiastic)$")
...
class ResumeSearchRequest(BaseModel):
    ...
    sort_by: str = Field(default="created_at", regex="^(created_at|updated_at|title|match_score)$")
...
class AnalysisSearchRequest(BaseModel):
    ...
    created_after: Optional[datetime] = None
    created_before: Optional[datetime] = None
    sort_by: str = Field(default="created_at", regex="^(created_at|match_score|processing_time)$")
    sort_order: str = Field(default="desc", regex="^(asc|desc)$")
    page: int = Field(default=1, ge=1)
    page_size: int = Field(default=20, ge=1, le=100)
...
class JobSearchRequest(BaseModel):
    ...
    is_active: bool = True
    sort_by: str = Field(default="posted_at", regex="^(posted_at|title|view_count)$")
    sort_order: str = Field(default="desc", regex="^(asc|desc)$")
    page: int = Field(default=1, ge=1)
    page_size: int = Field(default=20, ge=1, le=100)
...
class DataExportRequest(BaseModel):
    ...
    export_type: str = Field(..., regex="^(resumes|analyses|cover_letters|all)$")
...
class FeedbackCreateRequest(BaseModel):
    ...
    type: str = Field(..., regex="^(analysis_feedback|feature_request|bug_report|general)$")
    title: str = Field(..., min_length=1, max_length=255)
